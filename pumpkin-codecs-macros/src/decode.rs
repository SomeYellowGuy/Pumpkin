...
use syn::{Data, DeriveInput, Error, Fields, LitBool, LitStr, Ident};
...
pub fn derive_decode(codecs_crate: &Ident, input: &DeriveInput) -> Result<TokenStream, Error> {
    ...
    match &input.data {
        Data::Struct(data) => {
            ...
            let variant_decode = derive_single_variant_decode(codecs_crate, &name, &data.fields, &quote! { Self });
            Ok(quote! {
                impl #codecs_crate::codec::Decode for #name {
        ...
        Data::Enum(data) => {
            ...
            for variant in &data.variants {
                ...
                let variant_decode = if variant.fields.is_empty() {
                    ...
                    derive_single_variant_decode(codecs_crate, &name, &variant.fields, &qualified_variant_ident)
                };
...
fn derive_single_variant_decode(codecs_crate: &Ident, variant_ident: &Ident, fields: &Fields, variant_tokens: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
...
fn decode_field_tokens(codecs_crate: &Ident, field: ParsedField, counter: &mut usize) -> Result<DecodeFieldData, Error> {
